...
tf.flags.DEFINE_string("cell_type", "bilstm", "Type of rnn cell. Choose 'vanilla' or 'lstm' or 'gru' (Default: vanilla)")
tf.flags.DEFINE_string("word2vec", "../pubmed.vec", "Word2vec file with pre-trained embeddings")
...
tf.flags.DEFINE_boolean("bidirectional", True, "Bidirectional RNN (Default: True)")
...
def train():
    with tf.device('/cpu:0'):
        ...
        data, FLAGS.max_sentence_length = data_reader.read_data()
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # Generate batches
            batches = data_reader.batch_iter(list(zip(x_train, y_train)), FLAGS.batch_size, FLAGS.num_epochs)
