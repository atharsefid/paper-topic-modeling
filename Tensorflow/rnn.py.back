...
class RNN:
    def __init__(self, sequence_length, num_classes, vocab_size, embedding_size,
                 cell_type, hidden_size, l2_reg_lambda=0.0, bidirectional= True):
        ...
        with tf.name_scope("rnn"):
            ...
            if bidirectional:
                ...
                try:
                    all_outputs, _, _ = tf.nn.static_bidirectional_rnn(lstm_fw_cell, lstm_bw_cell, self.embedded_chars , dtype=tf.float32)
                except Exception: # Old TensorFlow version only returns outputs not states
                    all_outputs = tf.nn.static_bidirectional_rnn(lstm_fw_cell, lstm_bw_cell, self.embedded_chars, dtype=tf.float32)
            else:
